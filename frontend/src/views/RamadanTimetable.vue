<template>
    <div class="p-5 border-b-8 border-gray-100 flex flex-col items-center">
        <h1 class="text-2xl font-bold mb-4">Ramadan Timetable</h1>
        <div class="card flex flex-col md:flex-row gap-6">
            <!-- Calendar Field -->
            <div class="flex flex-col w-full">
                <label for="date" class="font-bold mb-1">Select Year</label>
                <Calendar
                    id="date"
                    :minDate="minDate"
                    :maxDate="maxDate"
...
const fetchSalatTimes = async () => {
    if (date.value && selectedLocation.value) {
        try {
            const response = await salatTimeService.getMRamadanSalatTime(
                date.value,
